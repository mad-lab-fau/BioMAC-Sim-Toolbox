...
% ======================================================================
function problem = running2D(model, resultfile, trackingData, initialGuess)
...
problem.addConstraint(@muscleDynamicConstraints,repmat(model.constraints.fmin(strcmp(model.constraints.type, 'muscles')),1,nNodes),repmat(model.constraints.fmax(strcmp(model.constraints.type, 'muscles')),1,nNodes), trackingData.extractData('angle'), trackingData.extractData('dur'),1)

% Constrain joint moments
problem.addConstraint(@jointMomentConstraint,zeros(height(model.joints)*nNodes,1),zeros(height(model.joints)*nNodes,1), trackingData.extractData('moment'), trackingData.extractData('angle'), trackingData.extractData('dur'),1)
