% ======================================================================
%> @file @Gait2d_osim/simuMarker.m
%> @brief Gait2d_osim function to simulates markers placed on 2D model
%> @details
%> Details: Gait2d_osim::simuMarker()
...
%> @param q           Double array: Generalized coordinates (Gait2d_osim.nDofs x 1)
...
%> @retval dm_dq      Sparse Jacobian dm/dq (variables.nVars.all x Gait2d_osim.nDofs)
%======================================================================
...
for iVar = 1 : nVars
    ...
    idxp = (iSegment-2)*6 + (1:2);	 % indices of position in FK
    idxR = (iSegment-2)*6 + (3:6); % indices of orientation in FK
    ...
    if nargout == 2
        ...
        elseif dlocal(3)
            dm_dq(iVar, :) = 0; % No Z direction --> dZ/dt = 0
        end
