%======================================================================
%> @file @Gait2dc/getSameRandomParticipant.m
%> @brief Gait2dc function to generate a participant with repeatable random muscle parameters
%> @details
%> Details: Gait2dc::getSameRandomParticipant()
%>
%> @author Marlies Nitschke, Eva Dorschky, Anne Koelewijn
%> @date June 2020
%======================================================================
...
%> @param  SDmusParamVector  Double vector: nParams (or larger) xnMus vector of normalized deviations
%>                           from the mean.
%>
%> @param  randModel   Gait2dc model object: New model generated using excelfile, 
%>                     bodyheight and bodymass of obj. Afterwards muscle parameters 
%>                     were changed randomly following a normal distribution.
%======================================================================
function  randModel = getSameRandomParticipant(obj, SDmusParamVector)
...
for iParam = 1 : length(musParam)
    ...
    for iMus = 1 : size(i_muscles, 1)
      ...
      if iParam == 1
          theMean = theMean*SDmusParamVector(end,iMus); %Scale Fmax to body mass
      end
      % Set value for right muscle
      randMus.(curParam)(curMusRight) = theMean + SDmusParamVector(iParam, iMus)*abs(theMean);
