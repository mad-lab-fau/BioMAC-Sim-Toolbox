%======================================================================
%> @file @Gait3d/computeMomentArms.m
%> @brief Gait3d function to generate  muscle length polynomials from Opensim muscle paths
%> @details
%> Details: Gait3d::computeMomentArms()
%>
...
%> @retval momentarm_model	  Struct: Resulting polynomials which are also saved at strrep(which(settings.osimfile), '.osim', '_momentarms.mat')
%======================================================================
...
settings.steps = 6;				% the number of steps for the range of motion of each dof
settings.order = 4; 			% max. polynomial order
settings.relerror = 0.05;		% when relative error in moment arm is below this level, no more terms are added
...
fid = fopen('gait3d_momentarms.log','w');
...
%=========================================================
    function [poly] = muscle_poly(obj, mus, range_muscleMoment, settings)
        ...
        fid1 = fopen('gait3d_momentarms.log','a');
...
%=========================================================
    function [momArms,zerolength] = opensim_get_momentarms(osimfile, angles, Mus, Dofs)
        ...
        % Set geometries to avoid warnings
        tofind = 'Geometry'; % Folder name containing all the geometries => Has to be in Matlab's search path
        dirs = regexp(path, pathsep,'split');          %cell of all individual paths
        temp = unique(cellfun(@(P) strjoin(P(1:find(strcmp(tofind, P),1,'last')),filesep), regexp(dirs,filesep,'split'), 'uniform', 0));    %don't let the blue smoke escape
        fullpaths = temp(~cellfun(@isempty,temp));     %non-empty results only
        if numel(fullpaths)==1
            ModelVisualizer.addDirToGeometrySearchPaths(fullpaths{1});
        else
            warning('Not able to find a unique opensim geometry folder of opensim in the matlab search path. .vtp files will not be found which causes an OpenSim warning.');
        end
...
%==========================================================
    function examine_momarms(mus, angles, ma_osim, ma_poly);
        ...
        for i=1:ndof;
            ...
            plot([ma_osim(:,i) ma_poly(:,i) ma_osim(:,i)-ma_poly(:,i)], '.-');
            if (i==ndof)
